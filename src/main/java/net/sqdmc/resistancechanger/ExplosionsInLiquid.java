...
public class ExplosionsInLiquid {
     ...
     */
    public static void Handle(EntityExplodeEvent event, ResistanceChanger plugin) {
        RC = plugin;
        ...
        for (int x = -RADIUS; x <= RADIUS; x++)
            for (int y = -RADIUS; y <= RADIUS; y++)
                for (int z = -RADIUS; z <= RADIUS; z++) {
                    ...
                    if (ResistanceChanger.isHookedFactions()) {
                    ...
                    if (ResistanceChanger.isHookedWorldGuard()) {
                        try {
                            RegionManager regionManager = RC.getWorldGuard().getRegionManager(targetLoc.getWorld());
